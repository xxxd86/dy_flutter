...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    tkController.addListener(
      () {
        if (tkController.value == TikTokPagePositon.middle) {//切换后判断时评的位置然后进行暂停
        ...
        children: <Widget>[
          PageView.builder(
            ...
            itemBuilder: (context, i) {
              // 拼一个视频组件出来
              bool isF = SafeMap(favoriteMap)[i].boolean;//判断是否点击赞
              var player = _videoListController.playerOfIndex(i)!;//当前视频
              var data = player.videoInfo!;//视频信息
              // 右侧按钮列
